...
document.addEventListener("DOMContentLoaded", () => {
  const el = document.getElementById("report");
  el.innerHTML = renderSweep(REPORTS);
  animateBars(el);
});`;
  return page("JVM Pulse — sweep", css, renderJs, bodyScript);
}

/** Render + write sweep.html (into the last run's directory) for 2+ runs. */
export async function writeStaticSweep(reports) {
  const html = await renderStaticSweepHtml(reports);
  const outPath = join(reports[reports.length - 1].artifacts.dir, "sweep.html");
  await writeFile(outPath, html);
  return outPath;
}
