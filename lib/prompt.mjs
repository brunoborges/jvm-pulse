...
/**
 * Build the optional troubleshooting sections (lock contention, safepoints,
 * exceptions, threads, slow I/O). Each is emitted only when the JFR recording
 * actually contains that signal, so the prompt stays focused on real problems.
 */
function buildSignalSections(jfr) {
    const con = jfr.contention || {};
    const spts = jfr.safepoints || {};
    const exc = jfr.exceptions || {};
    const thr = jfr.threads || {};
    const io = jfr.io || {};
    const sections = [];

    if (con.available) {
        sections.push(`## Lock contention (JFR — threads blocked entering monitors, ≥ threshold)
- ${num(con.count)} blocking events, ${ms(con.totalMs)} total blocked, max ${ms(con.maxMs)}
${topList(con.byMonitor, (m) => `${m.name} — ${ms(m.totalMs)} across ${num(m.count)} blocks (max ${ms(m.maxMs)})`)}`);
    }
    if (spts.available) {
        sections.push(`## Safepoints (time-to-safepoint synchronization)
- ${num(spts.count)} safepoints, total TTSP ${ms(spts.totalMs)}, avg ${ms(spts.avgMs)}, max ${ms(spts.maxMs)}`);
    }
    if (exc.available) {
        sections.push(`## Exceptions & errors
- Throwables created: ${num(exc.total)} (errors: ${num(exc.errors)})
${topList(exc.bySite, (x) => `${x.name} — ${num(x.count)}`)}`);
    }
    if (thr.available) {
        sections.push(`## Threads
- Peak ${num(thr.peak)}, ${num(thr.current)} active at end`);
    }
    if (io.available) {
        const ioLine = (label, agg) =>
          agg && agg.available ? `- ${label}: ${num(agg.count)} slow ops, ${ms(agg.totalMs)}, ${num(agg.bytes / (1024 * 1024), 1)} MB` : null;
        const lines = [
          ioLine("Socket read", io.socketRead),
          ioLine("Socket write", io.socketWrite),
          ioLine("File read", io.fileRead),
          ioLine("File write", io.fileWrite),
        ].filter(Boolean);
        if (lines.length) sections.push(`## Slow I/O (blocking ops over the JFR threshold)\n${lines.join("\n")}`);
    }
...
- Command: ${report?.command || "(not recorded)"}
- GC log: ${report?.source?.gcLogName || "?"}
- Source code: ${report?.source?.sourceCodePath || "(not provided)"}
...
3. **Allocation / code hotspots** — from the top allocations and hot methods,
   ...
   When a source-code path is listed above, inspect those files and cite the relevant
   classes and methods rather than stopping at JFR symbol names.
4. **Concurrency & latency signals** — if lock contention, safepoint, exception, or
   slow-I/O data is present above, call out anything hurting latency or throughput
   beyond GC (e.g. a hot lock, exception storm, or blocking I/O) and how to fix it.
...
4. When the run finishes, **call the \`jvm_pulse_ingest\` tool** with the absolute
   \`gcLogPath\` and (if produced) \`jfrPath\`, a short \`label\` describing the
   workload and any notable flags (heap size, collector), and the exact
   \`command\` you used to launch the workload — the full command line including
   every JVM flag (heap, collector, GC logging, JFR, etc.). The command is stored
   with the run so the user can see and compare the flags across runs. That parses
   and visualizes the data in the canvas.
