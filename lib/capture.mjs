...
export async function injectLaunch({ command, args = [], outDir, jfrMaxMb, durationMs, cwd }) {
  ...
  // place, or a --cwd different from this process's own leaves the JVM
  // looking for a directory that was never created.
  await mkdir(resolve(cwd ?? process.cwd(), outDir), { recursive: true });
...
export async function attach({ pid, transport = { type: "local" }, outDir, jfrMaxMb = 100, durationMs }) {
  ...
  // Validate pure arguments before any I/O (docker exec round trips in
  // particular) — otherwise a forgotten --duration on a docker attach burns
  // a preflight + pid-discovery call only to fail on something checkable
  // instantly, and a docker connectivity error masks the real problem.
  if (!durationMs) {
  ...
  if (transport.type === "docker") {
    ...
    await preflightDockerJcmd(transport.container);
    pid = await discoverContainerJavaPid(transport.container);
  }
  ...
  const remoteGcLog = transport.type === "docker" ? "/tmp/pulse-gc.log" : join(absOutDir, "gc-attach.log");
  const remoteJfr = transport.type === "docker" ? "/tmp/pulse-dump.jfr" : join(absOutDir, "dump-attach.jfr");
  ...
  if (transport.type === "docker") {
    ...
    return { pid, gcLogPath: localGcLog, jfrPath: localJfr };
  }
  return { pid, gcLogPath: remoteGcLog, jfrPath: remoteJfr };
}
...
async function discoverContainerJavaPid(container) {
  try {
    ...
    if (pids.length > 1) {
      throw new Error(`multiple java processes in container "${container}" (pids: ${pids.join(", ")}) — pass one explicitly with --pid.`);
    }
  } catch (e) {
