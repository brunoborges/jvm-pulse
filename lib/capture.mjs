...
export async function attach({ pid, transport = { type: "local" }, outDir, jfrMaxMb = 100, durationMs }) {
  ...
  const remoteGcLog = transport.type === "docker" ? "/tmp/pulse-gc.log" : join(outDir, "gc-attach.log");
  const remoteJfr = transport.type === "docker" ? "/tmp/pulse-dump.jfr" : join(outDir, "dump-attach.jfr");
  ...
  if (transport.type === "docker") {
    const localGcLog = join(outDir, "gc-attach.log");
    const localJfr = join(outDir, "dump-attach.jfr");
...
async function preflightDockerJcmd(container) {
  try {
    await execFileAsync("docker", ["exec", container, "which", "jcmd"]);
  } catch {
    throw new Error(
...
async function discoverContainerJavaPid(container) {
  ...
  // for a single-process container entrypoint (confirmed against
  // rating-engine's own jib-built image: java runs as PID 1 in-container).
  return "1";
}
