...
const RUNS_ROOT = join(EXT_DIR, "runs");
const ANALYZER = join(EXT_DIR, "tools", "GcLogAnalyzer.java");

// The extension is installed once (its files live at a single path shared by every
// session), so a single `runs/` directory would mix analyses from unrelated
// projects. Scope each workspace to its own subdirectory so opening the canvas in
// one project never surfaces another project's runs.
let runsDir = RUNS_ROOT;

/**
 * Point the analyzer at a per-workspace runs directory. Call this once the
 * session's workspace path is known. Passing a falsy value keeps the shared root
 * (used only as a fallback when no workspace is available).
 */
export function configureWorkspace(workspacePath) {
    if (workspacePath) {
        const key = createHash("sha1").update(String(workspacePath)).digest("hex").slice(0, 16);
        runsDir = join(RUNS_ROOT, `${basename(String(workspacePath))}-${key}`);
    } else {
        runsDir = RUNS_ROOT;
    }
    return runsDir;
...
const runsRoot = () => runsDir;
const latestJson = () => join(runsDir, "latest.json");

export { runsRoot as RUNS_DIR, latestJson as LATEST_JSON };
 ...
 *
 * @param {object} args { gcLogPath, jfrPath?, sourcePath?, label? }
...
export async function analyzeArtifacts(args = {}, onProgress = () => {}) {
    const { gcLogPath, jfrPath, sourcePath, label, command } = args;
    ...
    // A dump that FAILED inside the JVM (e.g. "Dump failed. Could not copy
    // recording data" from an empty/unusable JFR repository — jcmd exits 0
    // for that) still leaves a 0-byte .jfr behind. Treat it as absent so the
    // report says so instead of silently rendering empty JFR panels.
    const jfrOnDisk = jfrPath && existsSync(jfrPath);
    const haveJfr = jfrOnDisk && statSync(jfrPath).size > 0;
    ...
    if (jfrOnDisk && !haveJfr) {
        jfrData.error =
            `JFR recording at ${jfrPath} is empty (0 bytes) — the JVM's dump failed even though jcmd exited 0. ` +
            `Commonly the target JVM's JFR repository directory is empty or unwritable ` +
            `(check its -XX:FlightRecorderOptions=repository setting).`;
    }
    if (jfrFile) {
    ...
    const report = {
        ...
        source: {
            ...
            sourceCodeName: sourceDir ? basename(sourcePath) : null,
        },
...
export async function loadLatest() {
    try {
        return JSON.parse(await readFile(latestJson(), "utf8"));
    } catch {
        return null;
    }
}
...
// that shape so a caller-supplied id can never escape the runs directory.
...
export async function listRuns() {
    ...
    for (const ent of entries) {
        ...
        const reportPath = join(runsRoot(), ent.name, "report.json");
...
export async function loadRun(runId) {
    ...
    try {
        return JSON.parse(await readFile(join(runsRoot(), runId, "report.json"), "utf8"));
    } catch {
        return null;
    }
}
