...
export function resolvePosixShell() {
    ...
    const candidates = [
        ...
        // scoop's per-user install location (chocolatey defaults to the
        // Program Files paths above, already covered).
        join(homedir(), "scoop", "apps", "git", "current", "bin", "bash.exe"),
        join(homedir(), "scoop", "apps", "git", "current", "usr", "bin", "bash.exe"),
    ];
